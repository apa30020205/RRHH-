<?php
/**
 * Crear Funcionario
 * Sistema RRHH
 */

require_once __DIR__ . '/../../roles_rrhh/middleware/auth_middleware.php';
require_once __DIR__ . '/../../config/constants.php';
require_once __DIR__ . '/../../includes/functions.php';
require_once __DIR__ . '/../../classes/Database.php';
require_once __DIR__ . '/../../roles_rrhh/classes/Auth.php';

$pageTitle = 'Crear Funcionario - Sistema RRHH';

// Procesar formulario
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $db = Database::getInstance()->getConnection();
        
        // Validar cédula - ANTES de sanitizar para no perder formato
        $cedula = trim($_POST['cedula']);
        
        // Validar formato primero
        if (empty($cedula)) {
            throw new Exception("La cédula es obligatoria");
        }
        
        if (!validarCedula($cedula)) {
            throw new Exception("Cédula inválida. Formato aceptado: numérica (8-1234-5678) o alfanumérica (PE-123456-7). Solo se permiten letras, números y guiones.");
        }
        
        // Normalizar cédula (eliminar guiones, convertir a mayúsculas)
        $cedulaNormalizada = normalizarCedula($cedula);
        
        // Validar nuevamente después de normalizar (doble verificación)
        if (!validarCedula($cedulaNormalizada)) {
            throw new Exception("Error al procesar la cédula. Por favor, verifique el formato.");
        }
        
        // Verificar si la cédula ya existe
        $stmtCheck = $db->prepare("SELECT cedula FROM funcionarios WHERE cedula = ?");
        $stmtCheck->execute([$cedulaNormalizada]);
        if ($stmtCheck->fetch()) {
            throw new Exception("La cédula ya está registrada en el sistema");
        }
        
        // Calcular edad si se proporciona fecha de nacimiento
        $edad = !empty($_POST['fecha_nacimiento']) 
            ? calcularEdad($_POST['fecha_nacimiento']) 
            : intval($_POST['edad']);
        
        $stmt = $db->prepare("
            INSERT INTO funcionarios 
            (cedula, nombre, apellido, fecha_nacimiento, edad, sangre, no_posicion, 
             posicion_funcional, fecha_inicio, sede_provincia, Direccion)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        
        $stmt->execute([
            $cedulaNormalizada,
            sanitize($_POST['nombre']),
            sanitize($_POST['apellido']),
            $_POST['fecha_nacimiento'],
            $edad,
            sanitize($_POST['sangre']),
            intval($_POST['no_posicion']),
            sanitize($_POST['posicion_funcional']),
            $_POST['fecha_inicio'],
            sanitize($_POST['sede_provincia']),
            sanitize($_POST['Direccion'])
        ]);
        
        mostrarMensaje("Funcionario creado exitosamente", 'success');
        redirect(BASE_URL . '/pages/funcionarios/listar.php');
        
    } catch (PDOException $e) {
        if ($e->getCode() == 23000) {
            mostrarMensaje("La cédula o número de posición ya existe", 'error');
        } else {
            mostrarMensaje("Error al crear funcionario: " . $e->getMessage(), 'error');
        }
    } catch (Exception $e) {
        mostrarMensaje($e->getMessage(), 'error');
    }
}

include __DIR__ . '/../../includes/header.php';
?>

<div class="page-header">
    <h2>Crear Nuevo Funcionario</h2>
    <a href="<?php echo BASE_URL; ?>/pages/funcionarios/listar.php" class="btn">Volver</a>
</div>

<form method="POST" action="" data-validate>
    <div class="form-group">
        <label for="cedula">Cédula *</label>
        <input type="text" id="cedula" name="cedula" required maxlength="25" 
               placeholder="8-1234-5678 o PE-123456-7" 
               autocomplete="off">
        <small>Formato: numérica (8-1234-5678) o alfanumérica (PE-123456-7)</small>
    </div>
    
    <div class="form-group">
        <label for="nombre">Nombre *</label>
        <input type="text" id="nombre" name="nombre" required maxlength="40">
    </div>
    
    <div class="form-group">
        <label for="apellido">Apellido *</label>
        <input type="text" id="apellido" name="apellido" required maxlength="50">
    </div>
    
    <div class="form-group">
        <label for="fecha_nacimiento">Fecha de Nacimiento *</label>
        <input type="date" id="fecha_nacimiento" name="fecha_nacimiento" required>
    </div>
    
    <div class="form-group">
        <label for="edad">Edad</label>
        <input type="number" id="edad" name="edad" min="18" max="100" readonly>
        <small>Se calcula automáticamente</small>
    </div>
    
    <div class="form-group">
        <label for="sangre">Tipo de Sangre *</label>
        <input type="text" id="sangre" name="sangre" required maxlength="5" placeholder="O+, A-, etc">
    </div>
    
    <div class="form-group">
        <label for="no_posicion">Número de Posición *</label>
        <input type="number" id="no_posicion" name="no_posicion" required>
    </div>
    
    <div class="form-group">
        <label for="posicion_funcional">Posición Funcional *</label>
        <input type="text" id="posicion_funcional" name="posicion_funcional" required maxlength="45">
    </div>
    
    <div class="form-group">
        <label for="fecha_inicio">Fecha de Inicio *</label>
        <input type="date" id="fecha_inicio" name="fecha_inicio" required>
    </div>
    
    <div class="form-group">
        <label for="sede_provincia">Sede/Provincia *</label>
        <input type="text" id="sede_provincia" name="sede_provincia" required maxlength="20">
    </div>
    
    <div class="form-group">
        <label for="Direccion">Dirección *</label>
        <input type="text" id="Direccion" name="Direccion" required maxlength="100">
    </div>
    
    <div class="form-actions">
        <button type="submit" class="btn btn-primary">Guardar</button>
        <a href="<?php echo BASE_URL; ?>/pages/funcionarios/listar.php" class="btn">Cancelar</a>
    </div>
</form>

<script src="<?php echo BASE_URL; ?>/assets/js/validar_cedula.js"></script>
<script>
// Calcular edad automáticamente
document.getElementById('fecha_nacimiento').addEventListener('change', function() {
    const fechaNac = new Date(this.value);
    const hoy = new Date();
    const edad = hoy.getFullYear() - fechaNac.getFullYear();
    const mes = hoy.getMonth() - fechaNac.getMonth();
    const edadCalculada = (mes < 0 || (mes === 0 && hoy.getDate() < fechaNac.getDate())) ? edad - 1 : edad;
    document.getElementById('edad').value = edadCalculada;
});

// Validación adicional antes de enviar
document.querySelector('form').addEventListener('submit', function(e) {
    const cedulaInput = document.getElementById('cedula');
    const cedula = cedulaInput.value.trim();
    
    // Verificar que la cédula sea válida
    if (cedula.length < 8) {
        e.preventDefault();
        alert('Por favor, ingrese una cédula válida');
        cedulaInput.focus();
        return false;
    }
    
    // Verificar si hay mensaje de error visible
    const mensajeError = cedulaInput.parentNode.querySelector('.mensaje-cedula.error');
    if (mensajeError) {
        e.preventDefault();
        alert('Por favor, corrija el formato de la cédula antes de continuar');
        cedulaInput.focus();
        return false;
    }
    
    // Verificar si hay mensaje de duplicado
    const mensajeDuplicado = cedulaInput.parentNode.querySelector('.mensaje-cedula.warning');
    if (mensajeDuplicado) {
        e.preventDefault();
        alert('Esta cédula ya está registrada en el sistema. Por favor, verifique.');
        cedulaInput.focus();
        return false;
    }
});
</script>

<?php include __DIR__ . '/../../includes/footer.php'; ?>

